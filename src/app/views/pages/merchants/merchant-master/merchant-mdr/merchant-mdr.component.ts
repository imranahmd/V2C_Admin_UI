...
export class MerchantMDRComponent implements OnInit {
  ...
  public columnDefs: ColDef[] = [
    ...
    { field: 'sp_id', tooltipField: 'sp_id', headerName: 'Service Provider ID', filter: true },
    { field: 'sp_name', tooltipField: 'sp_name', headerName: 'Service Provider Name', filter: true },
    ...
    { field: 'min_amt', tooltipField: 'min_amt', headerName: 'Minimum Amount', filter: true },
    { field: 'max_amt', tooltipField: 'max_amt', headerName: 'Maximum Amount', filter: true },
    // {field: 'mdr_type',tooltipField: 'mdr_type', headerName: 'MDR Type', filter: 'agNumberColumnFilter'},
    { field: 'mdr_type_name', tooltipField: 'mdr_type_name', headerName: 'MDR Type', filter: true },

    { field: 'aggr_mdr', tooltipField: 'aggr_mdr', headerName: 'Aggregate MDR', filter: true },
    { field: 'reseller_mdr', tooltipField: 'reseller_mdr', headerName: ' Partner MDR', filter: true },
    { field: 'min_mdr', tooltipField: 'min_mdr', headerName: 'Minimum MDR', filter: true },
    { field: 'max_mdr', tooltipField: 'max_mdr', headerName: 'Maximum MDR', filter: true },
    { field: 'base_rate', tooltipField: 'base_rate', headerName: 'Bank Rate', filter: true },
    { field: 'min_base_rate', tooltipField: 'min_base_rate', headerName: 'Minimum Bank Rate', filter: true },
    { field: 'max_base_rate', tooltipField: 'max_base_rate', headerName: 'Maximum Bank Rate', filter: true },
    { field: 'start_date', tooltipField: 'start_date', headerName: 'Start Date', filter: true },
    { field: 'end_date', tooltipField: 'end_date', headerName: 'End Date', filter: true },
    // {field: 'reseller_mdr_type',tooltipField:'reseller_mdr_type', headerName: 'Reseller MDR Type', filter: 'agNumberColumnFilter'},
    { field: 'reseller_mdr_type_name', tooltipField: 'reseller_mdr_type_name', headerName: ' Partner MDR Type', filter: true },
    //  {field: 'bank_mdr_type',tooltipField:'bank_mdr_type', headerName: 'Bank MDR Type', filter: 'agNumberColumnFilter'},
    { field: 'bank_mdr_type_name', tooltipField: 'bank_mdr_type_name', headerName: 'Bank MDR Type', filter: true },

    // {field: 'instrument_brand',tooltipField:'instrument_brand', headerName: 'Instrument Brand'},
    { field: 'instrument_brand_name', tooltipField: 'instrument_brand_name', headerName: 'Instrument Brand', filter: true },
    { field: 'network_name', tooltipField: 'network_name', headerName: 'Instrument Type', filter: true },
    { field: 'surcharge_name', tooltipField: 'surcharge', headerName: 'Surcharge', filter: true },
    // { field: 'payout_by_name', tooltipField: 'Payout By', headerName: 'Payout By', filter: true },
    { field: 'mid', tooltipField: 'MID', headerName: 'MID', filter: true },
    { field: 'tid', tooltipField: 'TID', headerName: 'TID', filter: true },
    // { field: 'payout_escrow', tooltipField: 'payout_escrow', headerName: 'Payout Escrow',filter:true },
  ...
  ngOnInit(): void {
    ...
    this.merchantMDRform.get('netWork')?.setValidators([])
    this.merchantMDRform.get('netWork')?.updateValueAndValidity()
      ...
      .subscribe(params => {
        ...
        this.merchantService.getMerchant(params?.mid).subscribe((res: any) => {
          ...
          if (!this.resellerResponse || this.resellerResponse == '' || this.resellerResponse == '0') {
            ...
            // this.acc.expand('pan-2');
            // this.acc.expand('pan-3');
            // this.acc.expand('pan-4');
            //  this.acc.expand('pan-5');
    ...
    if (!(this.permissions.includes('Add New') || this.permissions.includes('Edit'))) {
      ...
      this.columnDefs.push({
        field: 'Action', resizable: true, cellRenderer: BtnCellRenderer, width: 25, pinned: 'right', cellRendererParams: [
          {
            clicked: async (field: any, param: any) => {
              this.tableInfo = param.data
              this.Edit(this.tableInfo)
  ...
  onSubmit(formvalue: any) {
    ...
    if (this.merchantMDRform.valid) {
      ...
      let MDRADD = {
        ...
        "payout": formvalue.payout||'1',
        "cardVariantName": "",
        "network":this.networkDropdown?formvalue.netWork:null,
        ...
        "instrumentBrand": this.ResinstrumentBrandDropdown ? formvalue.instrumentBrand : null,
      ...
      this.apiHttpService
        ...
        .subscribe((data) => {
          ...
          this.merchantMDRform.controls['netWork'].setValue("")
  ...
  async Edit(Editvalue: any) {
    ...
    setTimeout(() => {
      ...
      const patchData = {
        ...
        "instrument_id": Editvalue?.instrument_id || this.Resdatas?.find((inst: any) => inst?.FieldValue == Editvalue?.instrument_id) ? Editvalue?.instrument_id : '',
      ...
      this.merchantMDRform.patchValue(patchData);
      if (patchData?.instrument_id == "CC" || patchData?.instrument_id == "DC") {
        this.ResinstrumentBrandDropdown = true
        this.networkDropdown = false
        this.merchantMDRform.get('instrumentBrand')?.setValidators([Validators.required])
        this.merchantMDRform.get('instrumentBrand')?.updateValueAndValidity()
      }
      else if(patchData?.network=="0"||patchData?.network=="1")
       {
        this.networkDropdown = true
        this.ResinstrumentBrandDropdown = false
        this.merchantMDRform.get('netWork')?.setValidators([Validators.required])
        this.merchantMDRform.get('netWork')?.updateValueAndValidity()

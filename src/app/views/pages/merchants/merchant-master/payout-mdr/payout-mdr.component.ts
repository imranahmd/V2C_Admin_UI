...
@Component({
  selector: 'app-payout-mdr',
  templateUrl: './payout-mdr.component.html',
  styleUrls: ['./payout-mdr.component.scss']
})
export class PayoutMdrComponent implements OnInit {
  ...
  ngOnInit(): void {debugger
    this.columnDefs=   [
      ...
      { field: 'merchant_id', tooltipField: 'merchant_id', headerName: 'Auth ID', filter:true },
      { field: 'sp_id', tooltipField: 'sp_id', headerName: 'Service Provider ID', filter: true },
      { field: 'mop', tooltipField: 'MOP', headerName: 'MOP',filter:true },
      ...
      { field: 'aggr_mdr_Type', tooltipField: 'mdr_type_name', headerName: 'MDR Type', filter:true},
  
   
      { field: 'base_mdr_Type', tooltipField: 'base_mdr_Type', headerName: 'Base MDR Type', filter:true},
      { field: 'base_mdr', tooltipField: 'base_mdr', headerName: 'Base MDR ', filter: true },
      ...
      { field: 'DATE_FORMAT', tooltipField: 'DATE_FORMAT', headerName: 'End Date', filter:true },

      { field: 'MDR_apply_Type', tooltipField: 'MDR_apply_Type', headerName: 'MDR Apply Type', filter:true },
    
      { field: 'createdOn', tooltipField: 'createdOn', headerName: 'Created On', filter:true},
     
      { field: 'createdby', tooltipField: 'createdby', headerName: 'Created By', filter: true },
  
     
      { field: 'UpdatedOn', tooltipField: 'UpdatedOn', headerName: 'Updated On',filter:true },
      { field: 'IsDeleted', tooltipField: 'IsDeleted', headerName: 'IsDeleted',filter:true},
  ...
  onGridReady(params: GridReadyEvent<any>) {debugger
    ...
    return this.apiHttpService
      .post(
        `${API_URL}/Getpayoutmdr?pageSize=${this.pageSize}&page=${(this.currentPage - 1)}`, data
      )
      .subscribe((data:any)=>{
        this.rowData = data?.data  ,
         this.collectionSize = data?.data?.totalCount || data?.data?.length
      });
  ...
  onFilterChange(param: FilterChangedEvent | any) {
    ...
    // self.apiHttpService
    //   .post(
    //     `${API_URL}/Getpayoutmdr?pageSize=${this.pageSize}&page=${(this.currentPage - 1)}`, Requestbody
    //   )
    //   // @ts-ignore
    //   .subscribe((data) => {
  ...
  onSubmit(formvalue: any) {debugger
    ...
    if (this.merchantMDRform.valid) {
      ...
      let MDRADD = {
        "merchant_id":this.merchantId || formvalue.merchantId,
        "sp_id":formvalue.spId,
        "mop":formvalue.instrument_id,
        "aggr_mdr_Type":formvalue.mdrType,
        "aggr_mdr": formvalue.aggrMdr,
        "base_mdr_Type":formvalue.bankMdrType,
        "base_mdr":formvalue.baseRate,
        "start_date":formvalue.startDate,
        "end_date":formvalue.endDate,
        "id": this.updateID || '0',
        "mid":formvalue.mid,
        "tid":formvalue.tid,
        "min_amt":formvalue.minAmt,
        "max_amt": formvalue.maxAmt,
        "MDR_apply_Type":formvalue.serviceprovider
      }
      this.loading = true
      ...
      this.apiHttpService
        .post(
          `${API_URL}/insertpayoutmdr`, MDRADD
        )
        .subscribe((data) => {
          this.loading = false
          ...
          if (data) {
           this.alertService.successAlert(data.msg && (MDRADD.id == '0') ? "Merchant's Payout MDR Details Added Successfully.!" : "Merchant's MDR Details Updated Successfully.!");
          }
          this.rowData = data.data || Array.isArray(data) ? data : [];
          this.collectionSize = data.totalCount || data.length;
          ...
          this.apiHttpService
            .post(
              `${API_URL}/Getpayoutmdr?pageSize=${this.pageSize}&page=${(this.currentPage - 1)}`, this.randomdata
            )
            .subscribe((data) => {
              this.rowData = data?.data ,
              this.collectionSize = data?.data?.totalCount || data?.data?.length;
  ...
  async Edit(Editvalue: any) {
    ...
    setTimeout(() => {
      ...
      const patchData = {
        ...
        "mid": Editvalue.mid,
        "tid": Editvalue.tid,
        "startDate":Editvalue.start_date, 
        "endDate": Editvalue.end_date, 
        ...
        "bankMdrType": Editvalue.base_mdr_Type,
        ...
        "serviceprovider":Editvalue.MDR_apply_Type
      };
  ...
  delete(deletedata: any) {debugger
    let data = {
      "Id": deletedata.Id
    }

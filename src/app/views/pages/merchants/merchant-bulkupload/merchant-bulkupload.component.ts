...
// import { RiskMerchantFilter, ChartGraphsService } from '../chart-graphs.service';
import { ActivatedRoute, Router } from '@angular/router';
import { HttpClient } from '@angular/common/http';
import { MerchantService, RiskMerchantFilter } from '../merchant-master/merchant.service';
...
@Component({
  selector: 'app-merchant-bulkupload',
  templateUrl: './merchant-bulkupload.component.html',
  styleUrls: ['./merchant-bulkupload.component.scss']
})
export class MerchantBulkuploadComponent implements OnInit {
  ...
  onsubmit() {
    ...
    if (this.fileExtensionError == false && this.merchantform.valid) {
      if (this.file) {
        ...
        this.apiHttpService
          .post(
            `${API_URL}/create-merchant-bulkupload`, formData
          )
          .subscribe((data: any) => {debugger
            ...
            if(this.fileResponse?.message?.indexOf('Could not upload the file')>=0){
              this.fileResponse?.message ? this.alertService.errorAlert({ html: this.fileResponse?.message}) : this.alertService.errorAlert({ html: 'Blank Document is not allowed' })
              this.rowData=[]
            }else{
              this.fileResponse?.message ? this.alertService.successAlert(this.fileResponse?.message) : this.alertService.errorAlert({ html: 'Blank Document is not allowed' }),
              this.fileResponse?.message ? this.onVerify() : console.log(this.fileResponse?.message );
            }
  ...
  exportexcel(): void {
    ...
    this.exportAsExcelFile(this.rowData, this.fileName);

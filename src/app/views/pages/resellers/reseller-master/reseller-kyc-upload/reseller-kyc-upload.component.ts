...
export class ResellerKycUploadComponent implements OnInit, OnDestroy {
  @ViewChild('waverError') waverError: ElementRef
  @ViewChild('isWavierSelector') isWavierSelector: ElementRef
  @ViewChild('resellerKycDataSelector') resellerKycDataSelector: ElementRef
    ...
    this.route.queryParams
      .subscribe(params => {
          ...
          try {
            ...
            this.resellerService.getReseller(this.resellerId).subscribe((mData) => {

              this.resellerListData.push({
                reseller_name: mData[0]?.reseller_name,
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.status = this.resellerDetails?.kyc_approvel||this.resellerDetails[0]?.kyc_approvel
    ...
    this.subscriber$ = this.resellerService.kycUploadObserver.subscribe(async (data: any) => {
      ...
      try {
        if (data?.type == 'FILE_UPLOAD') {
          ...
          // console.log("File Upload Form===========>>>>>>", this.resellerKycDataArr()[outIndex])
        }

        if (data?.type == 'FILE_DELETE') {
          const {outIndex, innerIndex, type, fileIndex} = data?.fileData;
          if (!isNaN(innerIndex)) {
            const docListFArr = ((this.resellerKycDataArr()[outIndex]?.get('docTypeList') as FormArray)?.controls[innerIndex]?.get('docList') as FormArray);
            ...
            this.deleteFileService(
              ...
              this.currentSelectedGroup = {
                ...
                innerIndex,
                type,
                currentFormGroup: this.resellerKycDataArr()[outIndex]
              };
              // this.resellerService.uploadKycBrowseButton(this.currentSelectedGroup);
            })
          } else {
            // const controls = this.resellerKycDataArr()[outIndex]?.get('docList').controls;
            const docListFArr = (this.resellerKycDataArr()[outIndex]?.get('docList') as FormArray);
            const controls = docListFArr?.controls;
            this.deleteFileService(
              controls[fileIndex]?.get('docPath')?.value,
              controls[fileIndex]?.get('kycId')?.value,
  ...
  getDocListError(index: number) {
    ...
    if (type == 'multi') {
      let allFalse = true;
      // console.log(index,'---', this.resellerKycData.controls[index]);
    ...
    if (type == 'other') {
      ...
      const docTypeName = this.resellerKycData.controls[index]?.value?.document as String;
      console.log(index,'----------------=====>', docTypeName, docTypeName?.indexOf('Additional Documents'));
      (this.resellerKycData.controls[index].get('docTypeList') as FormArray)?.controls.forEach((listItem, innIndex) => {
        // console.log(index,'---', listItem.get('isSelected')?.value);
        if (listItem.get('isSelected')?.value) {
          allFalse = false;
      ...
      if (allFalse && !this.isWavier?.value && !(docTypeName?.indexOf('Additional Documents') >= 0)) {
        if (!error) error = {};
        error['required'] = true;
      }
    }
  ...
  public wavierValidator(): ValidatorFn {
    return (controls: AbstractControl) => {
      ...
      const resellerKycData = controls.get('resellerKycData') as FormArray;

      // console.log("In validator===>", isWavier?.value)
      // console.log(resellerKycData?.controls);
      let waiverError = true;

      if (isWavier?.value) {
        waiverError = !waiverError;
        wavierOffDate?.addValidators([Validators.required])
        wavierRemarks?.addValidators([Validators.required,moreThanOneWhiteSpaceValidator()])
      } else {
      ...
      resellerKycData?.controls.forEach((fbControl: any) => {
  ...
  onChangeWavier($event?: Event) {
    ...
    if (this.isWavier?.value) {
      this.wavierRemarks?.setValidators([Validators.required,moreThanOneWhiteSpaceValidator()])
      ...
      this.wavierOffDate?.updateValueAndValidity()
      this.resellerKycData.controls.forEach((listItem, index) => {
        if(this.resellerKycData.controls[index].value.document=='Additional Documents'){
          this.getDocList(index).controls.forEach((docListItem, listIndex) => {
            this.getDocListItem(index, listIndex).get('docPath')?.setValidators([])
            this.getDocListItem(index, listIndex).get('docName')?.setValidators([])
            this.getDocListItem(index, listIndex).get('docPath')?.updateValueAndValidity()
  ...
  reformatKycData(kycData: any[]) {
    ...
    kycData.forEach((kyc) => {
      ...
      if(this.status&&(this.status==1||this.status=='1'||this.status==2||this.status=='2')){
  ...
  addDocList(docType?: any) {
    if(docType?.document =='Additional Documents'){
      return new FormGroup({
        docName: new FormControl(docType?.docname || docType?.docName || '', []),
        docPath: new FormControl(docType?.docpath || docType?.docPath || '', []),
        kycId: new FormControl(docType?.kycId || 0),
        isSelected: new FormControl(true),
        uploadSuccess: new FormControl(false)
      });
      ...
      async () => {
        ...
        return Promise.all(docListControls?.controls.map((ctrl) => {
          ...
          return new Promise((resolve, reject) => {
            ...
            //   })
            // }, 2000)
            this.resellerService.deleteKycFile({
              resellerId: this.resellerId,
  ...
  resetSelection($event: MouseEvent, i: number, u: any) {
    const docListControls = (this.resellerKycData.controls[i]?.get('docList') as FormArray);
  ...
  downloadFileService(docname: any, docpath: any) {
    ...
    this.resellerService.downloadKycFile(formData)
      .subscribe((res: any) => {
        ...
        const NewfileName = "Rewardoo_" + today.getFullYear().toString() + "-" + ("0" + (today.getMonth() + 1)).slice(-2) + ("0" + today.getDate()).slice(-2) + "-" + random.toString().slice(-4) + fileInitial + "." + fileType;
  ...
  deleteFileService(docpath: any, kycId: any, docname: any) {
    ...
    return this.alertService.confirmBox(this.resellerService.deleteKycFile(formData), {
      ...
      this.currentSelectedGroup = {
        ...this.currentSelectedGroup,
        currentFormGroup: this.resellerKycDataArr()[this.currentSelectedGroup.outIndex]
      };
      this.resellerService.uploadKycBrowseButton(this.currentSelectedGroup);
    }, null, {

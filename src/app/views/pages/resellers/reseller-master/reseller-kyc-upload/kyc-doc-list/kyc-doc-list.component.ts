...
export class KycDocListComponent implements OnInit, OnDestroy {
  ...
  @Input('resellerId') resellerId: string;
  ...
  constructor(private resellerService: ResellerService, private alertService: AlertService) {
  }

  ngOnInit(): void {
    this.message = [];
    console.log("SUb compo======> ", this.selectedKycData)
    this.subscriber$ = this.resellerService.kycUploadObserver.subscribe((data: any) => {
  ...
  deleteFile(index: number) {
    ...
    this.resellerService.kycFileDeleteEvent({
    ...
    if (index === this.files.length) {
      ...
      this.resellerService.uploadKycFile(formData)
        ...
        .subscribe((data) => {
          ...
          this.resellerService.kycFileUploadComplete({
            outIndex: this.outIndex,
            innerIndex: this.innerIndex,
            type: this.type,
  ...
  prepareFilesList(files: Array<any>) {
    ...
    for (const item of files) {
      ...
      }else if (item.size > 30 * 1024 * 1024 ) {
    ...
    fileMaxSizeError && this.alertService.errorAlert({
      html: "Maximum 30 MB file is allowed"
    })
  ...
  downloadFile(i: number) {
    this.resellerService.kycFileDownloadEvent({
      outIndex: this.outIndex,
      innerIndex: this.innerIndex,
      type: this.type,
      fileIndex: i
  ...
  viewFile(i: number) {
    return this.resellerService.kycFileViewEvent({
      outIndex: this.outIndex,
      innerIndex: this.innerIndex,
      type: this.type,
      fileIndex: i

...
export class ResellerAddComponent implements OnInit {
  ...
  constructor(
    ...
    this.validationForm2 = this.formBuilder.group({
      ...
      brandName: ['', [moreThanOneWhiteSpaceValidator(), Validators.maxLength,Validators.pattern("^(?=.*[a-zA-Z]).+")]],
      ...
      expMonthlyTurnOver: ['', [Validators.required, Validators.pattern("^[1-9]\\d*(\\.\\d+)?$"),Validators.maxLength(10)]],
      avgAmountPerTransaction: ['', [Validators.required,Validators.pattern("^[1-9]\\d*(\\.\\d+)?$"),Validators.maxLength(10)]],
  ...
  async setFormValues(data: any) {debugger
    ...
    this.validationForm1.get('saleLead')?.patchValue(valueObj?.sales_lead==''?null:valueObj?.sales_lead)
  ...
  saleLeadList() {
    ...
    this.resellerService.getSaleLead(data).subscribe((res) => {

...
import { ColDef, FilterChangedEvent, FilterModifiedEvent, FilterOpenedEvent, GridApi, PaginationChangedEvent, RowNode, SelectionChangedEvent, ValueFormatterParams, } from 'ag-grid-community';
...
import { ReusableGridsComponent } from '../../recon-management/reusable-grids/reusable-grids.component';
...
import { ChargebackServiceService } from '../../charge-back/chargeback-service.service';
...
@Component({
  selector: 'app-charge-back-processing',
  templateUrl: './charge-back-processing.component.html',
  styleUrls: ['./charge-back-processing.component.scss']
})
export class ChargeBackProcessingComponent implements OnInit {
  ...
  tooltips: any = 'Comments is Required';
  ...
  constructor(
    ...
    private chargeBackService: ChargebackServiceService,
    fb: FormBuilder) {
    ...
    this.refundListFilterGroup = fb.group({
      ...
      merchantId: [, []],
      TransactionId: ["", [Validators.maxLength(40), moreThanOneWhiteSpaceValidator()]],
      searchFromDate: ["", []],
      searchToDate: ["", []],
    ...
    this.rowData.forEach((item) => {
      data.push(
        new FormGroup({
          ...
          bankCutoffDate: new FormControl(item.Bank_CutOff_Date),
          merchantCutoffDate: new FormControl(item.Merch_CutOff_Date),
          Comments: new FormControl(),
          AdminAction: new FormControl(),
          TxnId:new FormControl(item.TxnId),
          cbId:new FormControl(item.CBID),
  ...
  onRowSelected(event: any) {
    debugger
    var bookTypeIndex = event.node.id
    ...
    if (event.node.selected && bookTypeIndex) {
      currentControl.get('Select')?.setValue(true)

      this.refundRaiseLog[bookTypeIndex] = true
      // currentControl.get('AdminAction')?.enable();
      currentControl.get('AdminAction')?.setValidators([Validators.required, Validators.min(1)]);
      currentControl.get('Comments')?.setValidators([Validators.required, Validators.min(1)]);
      currentControl.get('bankCutoffDate')?.setValidators([Validators.required, Validators.min(1)]);
   
      currentControl.get('merchantCutoffDate')?.setValidators([Validators.required, Validators.min(1)]);
      
   
   
    } else {
      delete this.refundRaiseLog[bookTypeIndex]
      currentControl.get('Select')?.setValue(false)
      this.refundRaiseLog[bookTypeIndex] = false

      // currentControl.get('AdminAction')?.setValue(null);
      // currentControl.get('AdminAction')?.disable();
      currentControl.get('AdminAction')?.setValidators([]);
      currentControl.get('merchantCutoffDate')?.setValidators([]);
      currentControl.get('bankCutoffDate')?.setValidators([]);
      currentControl.get('Comments')?.setValidators([]);
      
    }


    currentControl.get('AdminAction')?.updateValueAndValidity();
    currentControl.get('merchantCutoffDate')?.updateValueAndValidity();
    currentControl.get('bankCutoffDate')?.updateValueAndValidity();
    currentControl.get('Comments')?.updateValueAndValidity();
  }
  ...
  ngOnInit(): void {
    ...
    this.columnDefs = [
      ...
      { field: 'CBID', tooltipField: 'CBID', headerName: 'Chargeback ID', filter: true },
      ...
      {
        field: 'Bank_CutOff_Date',
        headerName: 'Bank_CutOff_Date',
      ...
      {
        field: 'Merch_CutOff_Date',
        headerName: 'Merch_CutOff_Date',
        cellRendererFramework: ReusableGridsComponent,
        cellRendererParams: {
          ngTemplate: this.authorList,
        },
      ...
      {
        field: 'Dispute_Docs', headerName: 'Dispute_Docs', resizable: true, width: 25, pinned: 'right', cellRenderer: BtnCellRenderer, cellRendererParams: [
          {
            clicked: async (field: any, data: any, param: any, formvalue: any) => {
              debugger
              this.download = data.data.File_Name
    ...
    // this.chargeBackStatus = [{ FieldValue: 'RFD', FieldText: 'Request for Document' },
    // { FieldValue: 'RAD', FieldText: 'Request for Additional Document' },
    // { FieldValue: 'DSM', FieldText: 'Document Sent by Merchant' },
    // { FieldValue: 'DAA', FieldText: 'Document Acknowledged' },
    // { FieldValue: 'DSB', FieldText: 'Request Sent to Bank' },
    // { FieldValue: 'CBA', FieldText: 'Dispute Acknowledged & Refund Initiated' },
    // { FieldValue: 'CBR', FieldText: 'Dispute Rejected' }
  ...
  refreshGrid(formvalue: any) {
    ...
    let data = {
      ...
      "cbStatus": formvalue.Status || '',
      "fromDate": formvalue.searchFromDate || '',
      "toDate": formvalue.searchToDate || ''
    }


    this.chargeBackService.chargeBackProcessinglist(data).subscribe((data: any) => {
  ...
  ActionChange(event: any) {
    ...
    this.chargeBackService.adminActionBasedonDropDown(data).subscribe((res: any) => {
      this.actionData = res
    })
  }
  ...
  Getdropdown() {
    ...
    this.chargeBackService.getDropDown(data).subscribe((res: any) => {
      this.chargeBackStatus = res
    })
  }
  ...
  onSubmit(formvalue: any) {
    ...
    setTimeout(() => {
      ...
      if (this.refundListFilterGroup.valid) {
        ...
        this.rowData=[]
        this.chargeBackService.chargeBackProcessinglist(merchantdata)
          .subscribe((data: any) => {
            this.loading = false
        ...
        if (this.refundListFilterGroup.controls['Status'].invalid) {
    ...
    if (this.refundListFormGroup.valid) {
      ...
      const data = this.refundListFormGroup.get('allTransactions')?.value?.filter((ref: any) => ref.Select).map((ref: any) => {
        return {
          ...
          "cbId": ref.cbId,
          "action": ref.AdminAction
          ,
          "comments": ref.Comments
          ,
          "merchLastDate": ref.merchantCutoffDate,
          "bankLastDate": ref.bankCutoffDate,
          "TxnId": ref.TxnId
        }
      })
      var arr: any[] = []
      ...
      //     {
      //       "cbId": element.CBID,
      //       "action": element.Admin_Action
      //       ,
      //       "comments": element.Admin_Comments
      //       ,
      //       "merchLastDate": element.Merch_CutOff_Date,
      //       "bankLastDate": element.Bank_CutOff_Date,
      //       "TxnId": element.TxnId
      //     }
      //   )
      // });
      ...
      document?.getElementById('loading')?.classList.add("spinner-border")
      document?.getElementById('loading')?.classList.add("spinner-border-sm")
      this.chargeBackService.UpdateChargeBackData(data).subscribe((res: any) => {
        this.loading = false
        document?.getElementById('loading')?.classList.remove("spinner-border")
        document?.getElementById('loading')?.classList.remove("spinner-border-sm")
        if (res) {

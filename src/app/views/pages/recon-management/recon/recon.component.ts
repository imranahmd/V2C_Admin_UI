...
export class ReconComponent implements OnInit {
  ...
  constructor(private apiHttpService: ApiHttpService, private datepipe: DatePipe, private Services: RiskdashboardService, private alertService: AlertService, private reconservice: ReconServiceService) {
    ...
    this.uploadFileFormTwo = new FormGroup({
      Sp: new FormControl('', [Validators.required]),
      reconDate: new FormControl('', [Validators.required]),
      file: new FormControl('', [Validators.required]),
    })
  ...
  get form2() {
    return this.uploadFileFormTwo.controls;
  }
  ngOnInit(): void {
    this.ServiceProvide();
  }
  ...
  onGridReady(params: GridReadyEvent<any>) {
    ...
    return this.reconservice.getReconList(formData, this.pageSize, (this.currentPage - 1))
      ...
      .subscribe((data: any) => (this.rowData = data?.data || data||[], this.collectionSize = data?.totalCount || data?.length||0));
  ...
  onSubmit(formvalue: any) {
    ...
    var Date = this.datepipe.transform(formvalue.reconDate, 'yyyy/MM/dd');
    // this.refreshGrid(this.Date);
    let reconAddData = {
    ...
    if (this.uploadFileForm.valid) {
      this.reconservice.addReconFile(reconAddData).subscribe((data: any) => {debugger
        this.addResponse = data;
        if( this.addResponse?.error){
          this.alertService.errorAlert({
            html:  this.addResponse?.error 
          })
        }
        ...
        if (this.addResponse?.Status==true) {
          this.alertService.successAlert(this.addResponse?.message);
        }else{
          this.alertService.errorAlert({
            html:  this.addResponse?.message||"Unable to process file"
          })
        }
        this.refreshGrid(this.Date)
      })
  ...
  onNewSubmit(formvalue: any) {
    ...
    if (this.uploadFileFormTwo.valid) {
      this.reconservice.addReconFile(reconAddData).subscribe((data: any) => {debugger
        this.addResponse = data;
        if( this.addResponse?.error){
          this.alertService.errorAlert({

...
import { ColDef, GridApi, PaginationChangedEvent, SelectionChangedEvent, ValueFormatterParams, } from 'ag-grid-community';
...
@Component({
  selector: 'app-recon-upload-one',
  templateUrl: './recon-upload-one.component.html',
  styleUrls: ['./recon-upload-one.component.scss']
})
export class ReconUploadOneComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.columnDefs = [
      ...
      { field: 'MID', tooltipField: 'MID', headerName: 'Merchant ID', filter: true },
  ...
  onSubmit(formvalue: any) {
    ...
    setTimeout(() => {
      ...
      // if (this.refundListFilterGroup.controls['searchFromDate'].value != '') {
      ...
      // }
      // if (this.refundListFilterGroup.controls.searchToDate.value != '') {
  ...
  updateRefund() {
    ...
    if (this.refundListFormGroup.valid) {
      ...
      document?.getElementById('loading')?.classList.add("spinner-border")
      document?.getElementById('loading')?.classList.add("spinner-border-sm")
      this.chargeBackService.UpdateChargeBackData(arr).subscribe((res: any) => {
        this.loading = false
        document?.getElementById('loading')?.classList.remove("spinner-border")
        document?.getElementById('loading')?.classList.remove("spinner-border-sm")
        if (res) {

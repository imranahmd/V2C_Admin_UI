...
@Output() merchantSelectEvent: EventEmitter<any> = new EventEmitter<any>();
  ...
  constructor(private menuService: MenusService, private location: Location, private alertService: AlertService, private sideNavService: SideNavService, private ReconManagementService: ReconManagementService, private router: Router, private sidebarComponent: SidebarComponent, private storageService: StorageService) {
  ...
  onExportCSV() {
    ...
    if (this.searchFromDate) filter.startDate = this.searchFromDate;
    if (this.searchToDate) filter.endDate = this.searchToDate;
    return this.ReconManagementService.getAllMerchant(this.totalRecords + 3, 0, filter)
      .subscribe((data:any) => {
        const expData: any[] = [];
  ...
  onPaginationChange(param: PaginationChangedEvent | any) {
    ...
    if (this.searchFromDate) filter.startDate = this.searchFromDate;
    if (this.searchToDate) filter.endDate = this.searchToDate;
    this.ReconManagementService.getAllMerchant(this.pageSize, (param - 1),filter)
      .subscribe((data:any) => {
          this.rowData = data.data || data.merchants || data;
...
//
  ...
  onPageSizeChanges($event: Event) {
    ...
    if (this.searchFromDate) filter.startDate = this.searchFromDate;
    if (this.searchToDate) filter.endDate = this.searchToDate;
    this.ReconManagementService.getAllMerchant(this.pageSize, (this.currentPage - 1), filter)
      .subscribe((data:any) => (this.rowData = data.data || data.merchants || data, this.collectionSize = data.totalCount || data.totalRecords, this.gridApi.applyTransaction({add: this.rowData})));

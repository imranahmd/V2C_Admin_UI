...
@Component({
  ...
  template: `
    ...
    <div class="row">
      <div class="col" style="float:left" *ngFor="let conf of buttonConfig">
        <span
          type="text"
          *ngIf="run(conf.buttonVisible || 'true')"
          (click)="conf.clicked($event, params)"
        >
        {{ this.params.data[conf.buttonData] }}
        </span>
      </div>
      <!-- <div class="col" style="float:left" *ngFor="let conf of buttonConfig"> <button type="button" *ngIf="run(conf.buttonVisible || 'true')"
    [class]="'btn btn-outline-primary btn-icon '+conf.buttonClass" (click)="conf.clicked($event, params)">
{{conf.buttonText}}<i class="feather text-primary" [className]="'feather '+conf.buttonIconClass" disabled="conf.buttonDisable"></i>
</button></div> -->
    </div>
